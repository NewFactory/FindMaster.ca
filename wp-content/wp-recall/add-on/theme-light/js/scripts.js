...
function moveMenu() {
	LkMenu.append($('#light_ext_menu ul').html());
	$('#lk-menu .hideshow').remove();
	$('#light_ext_menu').remove();
}

// закрытие меню
function closeExtMenu(){
	if (RclOverlay.hasClass('light_mbl_menu')){ // проверяем что это наш оверлей
		RclOverlay.fadeOut(100).removeClass('light_mbl_menu');
	}
	$('#light_ext_menu').removeClass('bounce').css({'top' : '','right' : ''});
}
...
function menuPosition(){
	...
	$('#light_ext_menu').css({'top' : hUpMenu});
	...
	if (wRightMenu > 10) { // если у нас есть отступ и он не отрицательный - сдвигаем менюшку
		$('#light_ext_menu').css({'right' : wRightMenu});
	}
}
...
function alignMenu() {
	...
	$('body').append(													// формируем в кнопке контент
		'<div id="light_ext_menu"><ul>' + menuhtml + '</ul></div>'
	);
	...
	$('#lk-menu .hideshow').on('click', function () {
		$('#light_ext_menu').toggleClass('bounce', 500);
		RclOverlay.fadeToggle(100).toggleClass('light_mbl_menu'); // добавляем наш класс оверлею. Чтоб чужой не закрывать
		menuPosition();
	});

	RclOverlay.on('click', function () { closeExtMenu(); });
	$('#light_ext_menu').on('click', function () { closeExtMenu(); });
}

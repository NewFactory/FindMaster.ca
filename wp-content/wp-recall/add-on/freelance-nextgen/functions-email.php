...
function fng_send_email_about_insert_request( $request_id ) {
	...
	$text	 = '<p>' . __( 'К вашему заданию "' . $task->post_title . '" была добавлена новая заявка.' ) . '</p>'
...
function fng_send_email_about_new_comment( $comment_id ) {
	...
	$text	 = '<p>' . __( 'К вашей заявке для задания' ) . ' "' . $task->post_title . '" ' . __( ' заказчик добавил комментарий.' ) . '.</p>'
...
function fng_send_email_about_request_reject( $request_id ) {
	...
	$text	 = '<p>' . __( 'Ваша заявка к заданию' ) . ' "' . $task->post_title . '" ' . __( ' была отклонена его автором' ) . '.</p>'
...
function fng_send_email_about_request_take( $request_id, $request ) {
	...
	$text	 = '<p>' . __( 'Поздравляем, автор задания' ) . ' "' . $task->post_title . '" ' . __( ' утвердил вас исполнителем' ) . '.</p>';
...
function fng_send_email_about_performer_fail( $task_id ) {
	...
	$text	 = '<p>' . __( 'Исполнитель, назначенный ранее для выполнения задания' ) . ' "' . $task->post_title . '" ' . __( 'отказался от выполнения' ) . '.</p>'
...
function fng_send_email_about_task_expired( $task_id ) {
	...
	$text	 = '<p>' . __( 'Время отведенное вами на выполнение задания "' . $task->post_title . '" закончилось' ) . '.</p>'
	...
	$title	 = __( 'Задание просрочено' );
	$text	 = '<p>' . __( 'Время отведенное автором задания "' . $task->post_title . '" на его выполнение подошло к концу' ) . '.</p>'
...
function fng_send_email_about_performer_fired( $task_id ) {
	...
	$text	 = '<p>' . __( 'Автор задания' ) . ' "' . $task->post_title . '" ' . __( 'отказался от ваших услуг '
...
function fng_send_email_about_task_complete( $task_id ) {
	...
	$text	 = '<p>' . __( 'Автор задания' ) . ' "' . $task->post_title . '" ' . __( 'подтвердил его выполнение' ) . '.</p>'
...
function fng_send_task_chat_message( $messageData ) {
	...
	foreach ( $users as $userId ) {
		...
		$text	 = '<p>' . __( 'В рабочей области задания' ) . ' "' . $task->post_title . '" ' . __( 'появилось новое сообщение' ) . '.</p>'
...
function fng_send_email_about_claim( $task_id, $user_id, $text_claim ) {
	...
	$text	 = '<p>' . __( 'В задании' ) . ' "' . $task->post_title . '" ' . __( 'одним из участников была подана жалоба' ) . '.</p>'

...
if ( ! class_exists( 'main' ) ) {
	...
	class main {
		...
		public static function include_admins_script() {
			...
			wp_enqueue_style( 'css-amazon-jPages', plugins_url( '/css/jPages.css', dirname( __FILE__ ) ) );
			...
			wp_enqueue_script( 'js-amazon-s3-backup-jPages', plugins_url( '/js/jPages.js', dirname( __FILE__ ) ) );
		...
		public static function admin_inc_menu() {
			...
			add_menu_page(
				...
				'data:image/svg+xml;base64,' . base64_encode( file_get_contents( self::getPluginDir() . '/images/cg-logo.svg' ) )
			);
		...
		public static function backup_s3_impl() {
			self::remove( self::$log_file );
			$core = new core(
				'backup', array(
					'params' => array(),
			...
			if ( $res['size'] > 0 ) {
				...
				$res['data'] = array();
				if ( ! isset( $res['error'] ) ) {
					$res['error'] = 'Could not create backup';
				}
				$res['result'] = RESULT_ERROR;
			}
			$error_count = get_option( PREFIX_CODEGUARD . 'error_count' );

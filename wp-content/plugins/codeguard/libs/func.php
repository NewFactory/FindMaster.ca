...
if ( ! function_exists( 'cg_plugin_list' ) ) {

	function cg_plugin_list() {
		...
		$pluginlists = sizeof( $all_plugins );
		if ( $pluginlists > 0 ) {
			return $pluginlists;
		}
	}
...
if ( ! function_exists( 'cg_total_posts' ) ) {
	function cg_total_posts() {
...
if ( ! function_exists( 'cg_site_size_exceeded' ) ) {
	function cg_site_size_exceeded() {
		$size       = cg_folder_size( ABSPATH );
		$size_limit = '524288000'; // 500 mb
		return $size > $size_limit;
	}
}

if ( ! function_exists( 'cg_get_space_count' ) ) {
	function cg_get_space_count() {
		$post_count   = cg_total_posts();
		$plugin_count = cg_plugin_list();

		if ( cg_site_size_exceeded() ) {

			echo '<div class="warning-message" style="background-image: url(' . plugins_url( '/images/stripes_bg.png', dirname( __FILE__ ) ) . '); -webkit-box-sizing: border-box; -moz-box-sizing: border-box; box-sizing: border-box;"><p>';

			echo '' . __( 'Your site has', 'codeguard' ) . ' <strong>(' . $post_count . ')</strong> ' . __( 'posts', 'codeguard' ) . ',  <strong>(' . $plugin_count . ')</strong> ' . __( 'plugins', 'codeguard' ) . ', ';

			echo '' . __( 'and the total WordPress file content size is', 'codeguard' ) . ' <strong>(' . cg_format_size( cg_folder_size( ABSPATH ) ) . ')</strong>.</p>';

			echo '<p style="margin: 0px;">' . __( "We have analyzed your content and believe your site would be better served by our SFTP + MySQL database backup solution, rather than the plugin. Please see this support article <a href='https://codeguard.zendesk.com/hc/en-us/articles/115000581066--How-To-Add-A-Website' target='_blank'>video-tutorial</a> for adding your website or contact support <a href='https://www.codeguard.com/support/'>https://www.codeguard.com/support/</a> if you need additional assistance.", 'codeguard' ) . '</p>';
...
if ( ! function_exists( 'cg_folder_size' ) ) {
	function cg_folder_size( $path ) {
		$total_size = 0;
			...
			foreach ( $files as $t ) {
				if ( '.' != $t && '..' != $t ) {
					...
					if ( is_dir( $currentFile ) ) {
						$size        = cg_folder_size( $currentFile );
						$total_size += $size;
					} else {
...
if ( ! function_exists( 'cg_format_size' ) ) {
	function cg_format_size( $size ) {
...
if ( ! function_exists( 'get_system_data' ) ) {
	function get_system_data() {
		...
		return array(
			...
			'php_verion'          => $php,
			'maxExecutionTime'    => $maxExTime,
			'extensions'          => $extensions,
			'disabledFunctions'   => $disabledFunctions,
			'mysqlVersion'        => $mysql,
			'upMaxExecutionTime'  => $upMaxExTime,
			'newMaxExecutionTime' => $newMaxExTime,
			'upMemoryLimit'       => $upMemLimit,
			'newMemoryLimit'      => $newMemLimit,
			'maxMemoryLimit'      => $maxMemLimit,
			'ex'                  => $ext,
			'func'                => $function,
		);
	}

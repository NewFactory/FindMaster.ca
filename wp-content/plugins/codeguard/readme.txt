...
CodeGuard protects your website with daily automatic backup and monitoring. Automatically fix mistakes or issues by restoring to a previous version.
...
CodeGuard helps should anything go wrong - deleted files are now recoverable, overwritten files are now obtainable, and if your site is hacked, the malware is easily removable.
...
1. Enter your CodeGuard API key from [the CodeGuard WordPress page](https://www.codeguard.com/wordpress-plugin) and click the update button.
...
The CodeGuard WordPress plugin can be configured to ignore certain files during backups and restores. 

By default, the plugin looks for blacklist patterns in a file named .codeguard_blacklist in the WordPress root directory. An alternate location and file name can be specified in the wp_options database table using the key "codeguard_backup_blacklist_file_path" and the absolute path to the blacklist file as the value. 

Each pattern uses glob-style syntax, and patterns are evaluated as absolute paths. There should be only one pattern per line. Relative paths can be specified using a "./" prefix. Relative paths are always relative to the WordPress root directory, regardless of the blacklist file location.

= Blacklist examples =
Exclude a single file:
/path/to/wordpress/wp-config.php

Exclude everything in a folder:
/path/to/wordpress/wp-admin/*

Exclude a path relative to the WordPress root directory:
./wp-config.php

Exclude everything in a folder relative to the WordPress root directory:
./wp-admin/*

Exclude all files of a particular type in a particular directory:
/path/to/images/*.jpeg
/path/to/images/*.jpg

Recursively exclude all files of a particular type in a particular directory:
/path/to/videos/**.webm

Globally exclude files of a particular type:
**.zip
...
= 0.74 =
* Resolved issue with pagination on backup list.
* Remove global ini_set directives.

= 0.73 =
* Improved backup and restore isolation.
* Updated error messages.
* Broader compatibility with blacklist functionality.
...
= 0.70 =
* Improved restore functionality and interface.
* Updated styles.

= 0.69 =
* Blacklist and restore improvements.

= 0.68 =
* Implement file blacklisting functionality.

= 0.67 =
* Update database connection process for greater compatibility.

= 0.66 =
* AWS SDK replaced for S3 operations and backward compatibility improved with PHP version 5.2 and above.

= 0.65 =
* Changes to enable single signon.

= 0.64 =
* Some generic styling issues fixed.

= 0.63 =
* For more accesability, plugin setting link added adminbar and plugin settings page.

= 0.62 =
* File stats fixed, backup file size increased for host memory.

= 0.61 =
* Fixes and php version compability.

= 0.60 =
* Total content size message implementation after the plugin activation.

= 0.59 =
* Upload more detailed logs
* Avoid uploading 0 byte backup pieces (occurs when disk space is low)

= 0.58 =
* Update for WordPress v4.5.1 compatibility.

= 0.57 =
* Update for WordPress v4.5 compatibility.
* Update menu location.

= 0.56 =
* Improve integration with main CodeGuard dashboard.
* Only grant users with the "update_core" capability access to the plugin.
* Simplify plugin interface.

= 0.55 =
* Integrate into the main CodeGuard dashboard.

= 0.54 =
* Update for v4.4 compatibility.

= 0.53 =
* Use more compatible array syntax, add troubleshooting tools.

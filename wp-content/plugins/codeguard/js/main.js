...
function blick(id, border_)
{
	...
	).animate( {outlineWidth: '0px',outlineColor: '#cd433d' } , 400 );
	if (blick_form) {
...
function recovery_form(type, name)
{
	...
	if (confirm( 'Are you sure you want to restore from the selected backup?' )) {
		...
		jQuery( '#restore-message > p' ).html(
			`
			<img id ="backup-spinner" style="vertical-align: middle; padding-right: 10px;" src="/wp-admin/images/loading.gif">
			A restore has started ! This may take a few moments. Please do not navigate away from this page until the restore completes.
			`
		);
		jQuery( "#restore-message" ).removeClass( 'failure-message' );
		jQuery( "#restore-message" ).addClass( 'success-message' );
		jQuery( "#restore-message" ).show( "slow" );
		...
		jQuery.ajax(
			...
			success: function(data){
				...
				jQuery( '#restore-message' ).hide();
				if (data.result == 'success') {
					jQuery( '.title-logs' ).css( 'display', 'none' );
					jQuery( '#restore-message' ).removeClass( 'failure-message' );
					jQuery( '#restore-message' ).addClass( 'success-message' );
					jQuery( '#restore-message > p' ).html( 'Successfully restored CodeGuard backup: ' + name );
				} else {
					jQuery( '.title-logs' ).css( 'display', 'none' );
					jQuery( '#restore-message' ).removeClass( 'success-message' );
					jQuery( '#restore-message' ).addClass( 'failure-message' );
					jQuery( '#restore-message > p' ).html( 'Failed to restore CodeGuard backup: ' + name );
				}
				jQuery( '#restore-message' ).show( "slow" ).delay( 15000 ).hide( 'slow' );
			},

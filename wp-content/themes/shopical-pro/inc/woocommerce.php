 ...
 * @since 1.0.0
 */
function shopical_autocomplete_scripts_enqueues()
{
    ...
    if ($search_autocomplete == 'yes') {
        ...
        wp_enqueue_script(
            'shopical-autocomplete',
            get_template_directory_uri() . '/assets/search-autocomplete-script.js',
            array('jquery'),
            '1.0.0',
            true
        );
...
add_action('wp_enqueue_scripts', 'shopical_autocomplete_scripts_enqueues');
...
function shopical_woocommerce_product_meta()
{

    global $product;

    $product_brands = wp_get_post_terms($product->get_id(), 'shopical_brand');

    echo wc_get_product_brand_list($product->get_id(), ', ', '<span class="posted_in">' . _n('Brand:', 'Brands:', count($product_brands), 'shopical') . ' ', '</span>');
}

add_action('woocommerce_product_meta_end', 'shopical_woocommerce_product_meta');
...
if (!function_exists('shopical_language_and_currency_switcher')) :
    ...
    function shopical_language_and_currency_switcher()
    {
        ...
        $currency_switcher_shortcode = shopical_get_option('aft_currency_switcher_shortcode');
        ...
        <ul class="af-my-account-menu prime-color">
            ...
            <?php if (is_user_logged_in()):
                ...
                <li><a class="af-my-account-log"
                       href="<?php echo esc_url(wc_get_account_endpoint_url($customer_logout)); ?>"><?php echo esc_html_e('Logout', 'shopical'); ?></a>
                </li>
            <?php endif; ?>
        </ul>
        <?php
